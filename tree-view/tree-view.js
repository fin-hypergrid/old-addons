...
TreeView.prototype.setRelation = function(join) {
    ...
    if (join) {
        behavior.setPipeline([
            include(this.options.includeFilter, require('./dataSources/DataSourceTreeviewFilter')),
            include(this.options.includeSorter, require('./dataSources/DataSourceTreeviewSorter')),
            require('./dataSources/DataSourceTreeview')
        ], {
            stash: 'default'
        });
    }
