...
module.exports = function (grid){
    ...
    var EC = grid.cellRenderers.get('GVEmptyCell'),
        GSC;

    GSC = EC.constructor.extend('GVSimpleCell', {

        paint: function(gc, config) {
            config = this.setBorderInfo(config);
            var val = config.value,
                plugin = grid.plugins.groupView,
                bounds = config.bounds2,
                x = bounds.x,
                y = bounds.y,
                width = bounds.width,
            ...
            //Row Handle Numbers Should be Based on rows with actual data
            if (config.isLeaf && config.isHandleColumn){
                var idx = plugin.directDataSourceLink.getChildViewByIndex(config.dataCell.y);
                val = idx + 1; //row column is 1-based
            } else if (config.isHandleColumn){
                val = '';
            }

            //Override the colors of cells
            if (config.isLeaf && !config.isTreeColumn && !config.isHandleColumn){
                config.backgroundColor = config.leafCellBackgroundColor;
            } else if (config.isTreeColumn && config.isHeaderRow){
                config.backgroundColor = config.rowHeaderBackgroundColor;
            } else if (config.isTreeColumn){
                config.backgroundColor = config.treeHeaderBackgroundColor;
            }
            ...
            this.paintBorder(gc, config);
        }
    });

    grid.cellRenderers.add('GVSimpleCell', GSC);
};
...
function renderMultiLineText(gc, config, val, leftPadding, rightPadding) {
    var x = config.bounds2.x,
        y = config.bounds2.y,
        width = config.bounds2.width,
        height = config.bounds2.height,
